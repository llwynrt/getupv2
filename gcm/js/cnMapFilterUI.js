   ...
   */
	function cnMFUI_init (options) {
		...
		function resizeSlider(elemId) {
			...
			var sWidth = getRtSideWidth() -50;/*- $('#resultsDataStatus').width() - 10;*/
		...
		/**
		 * affichage de la date dans les sliders
		 * passage au format français : Marie-Lyse Briffaud
		 */
		function sliderDate(nDays) {
			var day = new Date();
			day.setTime(day.getTime() + nDays*24*3600*1000); //expires in ndays days (milliseconds)
		  	return cnMF.formatDate(day, 'D-m');
		  	/* +'<br>'+cnMF.formatDate(day, 'd');
		  	TODO : afficher le jour en français*/
		}
		...
		function updateResultsTable(divId, onlyValidCoords, clearTable){
			...
			for (var i in cnMF.eventList) {
				...
				rowHTML += "<td>" + cnMF.formatDate(kk.dateStart, 'D/m/Y H:i') + "</td>"; // does sort 
	 ...
	 */
		...
		function cbCalendarLoad(calendarInfo) {
			...
			if (cnMF.validCalendars) {
				 ...
				 *localisation du texte et des dates
				 */
				$('#calendarTitleContent').html("<h3><a title='"+calendarInfo.desc+" - Cliquez pour voir le calendrier dans une nouvelle fenêtre' "
					+"class='jumpLink' target='_blank' href='"+ calendarInfo.gcLink +"'>"+ calendarInfo.gcTitle +"</a></h3>"
					+ '<span>Le calendrier a '+ calendarInfo.totalEvents + 
					...
					+' évènement(s) <nobr>du '+ cnMF.formatDate(startDate, 'D/m/Y') +'</nobr>'
					+' <nobr>au '+ cnMF.formatDate(endDate, 'D/m/Y')  +'</nobr>'
					+' &nbsp; </nobr>'
					+ '</span><div id="newDates"></div>');
